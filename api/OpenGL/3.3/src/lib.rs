...
pub unsafe extern "C" fn compile_ast(ast_ptr: *const ClockAST) -> TranspiledPipeline {
    ...
    if let Err(reason) = validate_feature_support(ast) {
        eprintln!("[Clock OpenGL 3.3] Transpilation canceled: {}", reason);
        return TranspiledPipeline {
            vertex_source: std::ptr::null_mut(),
            fragment_source: std::ptr::null_mut(),
        };
    ...
    vert_output.push('\n');
    vert_output.push_str("#version 330 core\n");
    vert_output.push('\n');
    frag_output.push_str("// This shader was made with Clock\n");
    frag_output.push('\n');
    frag_output.push_str("#version 330 core\n");
    frag_output.push('\n');
        ...
        for class in classes {
            ...
            for method in methods {
                if unsafe { is_method_decorated_as(method, Decorator::Tessellation) } {
                    return Err("[Tessellation] is not supported in OpenGL 3.3 core. Upgrade to a higher OpenGL version (like 4.0 or higher) to enable tessellation.");
                }
                if unsafe { is_method_decorated_as(method, Decorator::Compute) } {
                    return Err("[Compute] is not supported in OpenGL 3.3 core. Upgrade to a higher OpenGL version (like 4.3 or higher) to enable compute shaders.");
                }
            }
        }
    }
